...
prior = {row['pid'] for row in json.loads((evidence / 'module-edits-native.json').read_text())}
reports = []
for phase in ('prepare', 'restored'):
    adb('shell', 'am', 'force-stop', package)
    path = f'/sdcard/Download/pyfa-b04231-{phase}.json'
    adb('shell', 'rm', '-f', path)
    output = adb('shell', 'am', 'instrument', '-w', '-r', '-e', 'class', 'io.github.sussic.pyfa.ChargeEditingTest',
        '-e', 'b04231_phase', phase, f'{package}.test/io.github.sussic.pyfa.DiagnosticTestRunner', timeout=900)
    (evidence / f'charge-edits-{phase}-instrumentation.txt').write_text(output, encoding='utf-8')
    names = ('picker', 'loaded', 'script', 'crystal', 'mining', 'active', 'empty') if phase == 'prepare' else ('restored',)
    for name in (*names, 'failure'):
        picture = f'/sdcard/Download/pyfa-b04231-{name}.png'
        if subprocess.run(['adb', 'shell', 'test', '-f', picture], timeout=30).returncode == 0:
            ...
            (evidence / f'charge-edits-{name}.png').write_bytes(png)
    ...
    (evidence / f'charge-edits-{phase}-native.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
    for name in names:
        assert (evidence / f'charge-edits-{name}.png').is_file()
(evidence / 'charge-edits-native.json').write_text(json.dumps(reports, indent=2) + '\n', encoding='utf-8')
