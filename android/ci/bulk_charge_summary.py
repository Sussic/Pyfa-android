"""Require every native bulk charge observation and durable process restart."""
...
PREPARE = ['complete_matrix', 'all_four_recipients', 'prior_fits_unchanged', 'selection_recreation',
    'selected_ammo', 'asymmetric_skip', 'similar_variants', 'script_controls', 'empty_fit', 'clear_selection',
    'stale_selection_discarded', 'fit_switch', 'independent_copy', 'atomic_rejections', 'recent_unchanged', 'typed_protocol_guards']
RESTORED = ['fresh_process_restore', 'identities_inputs_values_retained', 'bulk_types_and_recent_retained', 'reopen_each_fit']
CODEC = ['decimal_index', 'decimal_revision', 'boolean_id', 'decimal_charge', 'candidate_out_of_range',
         'duplicate_candidate', 'decimal_candidate', 'missing_reference', 'wrong_selection_subset', 'extra_field']


def summarize(reports, engine):
    fixture = json.loads((Path(__file__).resolve().parents[2] / 'tools/android_reference/fixtures/bulk-charges.json').read_text())
    ...
    source = named['B05 bulk charges – 探索']; copied = named['B05 independent bulk copy']
    assert prepare['saved']['active_id'] == source['id']
    for value, copy in ((source, False), (copied, True)):
        rows = value['modules']
        exact(['Dual 150mm Railgun II', 'Dual 150mm Railgun I', '200mm Railgun II', 'Sensor Booster II'],
              [row['name'] for row in rows[:4]])
        exact([None if copy else 'Spike M', 'Iron Charge M', 'Spike M', 'Scan Resolution Script'],
              [row['charge'] for row in rows[:4]])
        exact(['ACTIVE', 'ACTIVE', 'OVERHEATED', 'ACTIVE'], [row['state'] for row in rows[:4]])
    # Options are integer IDs/positions only; never accept scalar normalization
    # which erases the strict transport's original numeric kinds.
    for report in reports:
        raw = report['options']
        expected_kinds = {}
        for i, fit in enumerate(raw['data']):
            ...
            for j, row in enumerate(fit['modules']):
                ...
                for key in ('index', 'item_id', 'charge_id'):
                    if row[key] is not None: expected_kinds[f'{prefix}.{key}'] = 'integer'
                for key in ('charge_ids', 'selection_candidates', 'similar_candidates'):
                    for k in range(len(row[key])): expected_kinds[f'{prefix}.{key}[{k}]'] = 'integer'
            for j in range(len(fit['items'])): expected_kinds[f'root[{i}].items[{j}].id'] = 'integer'
