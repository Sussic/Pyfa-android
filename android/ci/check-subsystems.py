"""Run B06.2 offline subsystem touch actions and a real process restart."""
import json
from pathlib import Path
import re
import subprocess
from subsystem_summary import summarize
...
prior = {row['pid'] for row in json.loads((evidence / 'hull-modes-native.json').read_text())}
reports = []
for phase in ('prepare', 'restored'):
    adb('shell', 'am', 'force-stop', package)
    path = f'/sdcard/Download/pyfa-b062-{phase}.json'
    adb('shell', 'rm', '-f', path)
    output = adb('shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
        'io.github.sussic.pyfa.SubsystemTest', '-e', 'b062_phase', phase,
        f'{package}.test/io.github.sussic.pyfa.DiagnosticTestRunner', timeout=900)
    (evidence / f'subsystems-{phase}-instrumentation.txt').write_text(output, encoding='utf-8')
    names = ('tengu-empty', 'offensive-added', 'offensive-replaced',
             'offensive-removed-invalid', 'vexor-empty') if phase == 'prepare' else ('restored-invalid-copy',)
    for name in (*names, 'failure'):
        picture = f'/sdcard/Download/pyfa-b062-{name}.png'
        if subprocess.run(['adb', 'shell', 'test', '-f', picture], timeout=30).returncode == 0:
            ...
            (evidence / f'subsystems-{name}.png').write_bytes(png)
    ...
    (evidence / f'subsystems-{phase}-native.json').write_text(
        json.dumps(report, indent=2) + '\n', encoding='utf-8')
    for name in names:
        assert (evidence / f'subsystems-{name}.png').is_file()
(evidence / 'subsystems-native.json').write_text(json.dumps(reports, indent=2) + '\n', encoding='utf-8')
