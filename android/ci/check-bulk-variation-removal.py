"""Run B05.4 offline touch actions and verify a real process restart."""
import json
from pathlib import Path
import re
import subprocess
from bulk_variation_removal_summary import summarize
...
prior = {row['pid'] for row in json.loads((evidence / 'clone-fill-native.json').read_text())}
reports = []
for phase in ('prepare', 'restored'):
    adb('shell', 'am', 'force-stop', package)
    path = f'/sdcard/Download/pyfa-b054-{phase}.json'
    adb('shell', 'rm', '-f', path)
    output = adb('shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
        'io.github.sussic.pyfa.BulkVariationRemovalTest', '-e', 'b054_phase', phase,
        f'{package}.test/io.github.sussic.pyfa.DiagnosticTestRunner', timeout=900)
    (evidence / f'bulk-variation-removal-{phase}-instrumentation.txt').write_text(output, encoding='utf-8')
    names = ('selected-preview', 'selected-saved', 'similar-preview', 'similar-saved',
             'removal-preview', 'removal-saved', 'empty') if phase == 'prepare' else ('restored',)
    for name in (*names, 'failure'):
        picture = f'/sdcard/Download/pyfa-b054-{name}.png'
        if subprocess.run(['adb', 'shell', 'test', '-f', picture], timeout=30).returncode == 0:
            ...
            (evidence / f'bulk-variation-removal-{name}.png').write_bytes(png)
    ...
    (evidence / f'bulk-variation-removal-{phase}-native.json').write_text(
        json.dumps(report, indent=2) + '\n', encoding='utf-8')
    for name in names:
        assert (evidence / f'bulk-variation-removal-{name}.png').is_file()
(evidence / 'bulk-variation-removal-native.json').write_text(json.dumps(reports, indent=2) + '\n', encoding='utf-8')
