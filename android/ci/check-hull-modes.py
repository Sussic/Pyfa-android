...
prior = {row['pid'] for row in json.loads((evidence / 'bulk-variation-removal-native.json').read_text())}
reports = []
for phase in ('prepare', 'restored'):
    adb('shell', 'am', 'force-stop', package)
    path = f'/sdcard/Download/pyfa-b061-{phase}.json'
    adb('shell', 'rm', '-f', path)
    output = adb('shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
        'io.github.sussic.pyfa.HullModeTest', '-e', 'b061_phase', phase,
        f'{package}.test/io.github.sussic.pyfa.DiagnosticTestRunner', timeout=900)
    (evidence / f'hull-modes-{phase}-instrumentation.txt').write_text(output, encoding='utf-8')
    names = ('confessor-initial', 'confessor-changed', 'vexor-empty') if phase == 'prepare' else ('restored-copy',)
    for name in (*names, 'failure'):
        picture = f'/sdcard/Download/pyfa-b061-{name}.png'
        if subprocess.run(['adb', 'shell', 'test', '-f', picture], timeout=30).returncode == 0:
            ...
            (evidence / f'hull-modes-{name}.png').write_bytes(png)
    ...
    (evidence / f'hull-modes-{phase}-native.json').write_text(
        json.dumps(report, indent=2) + '\n', encoding='utf-8')
    for name in names:
        assert (evidence / f'hull-modes-{name}.png').is_file()
(evidence / 'hull-modes-native.json').write_text(json.dumps(reports, indent=2) + '\n', encoding='utf-8')
