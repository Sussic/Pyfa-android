...
class BulkStatesTest {
    @get:Rule val compose = createAndroidComposeRule<MainActivity>()
    private val context get() = InstrumentationRegistry.getInstrumentation().targetContext
    private val picker get() = ViewModelProvider(compose.activity)[BulkStateEditorModel::class.java]
    ...
    private fun options(id: String) = EngineRuntime.bulkStateOptions(context, id).get(120, TimeUnit.SECONDS)
    private fun allOptions() = typed(array(fits().map { StateTestJson.options(options(it.id)).put("fit_id", it.id).put("revision", it.revision) }))
    ...
    private fun report(id: String, recipients: List<String> = emptyList()): JSONObject {
        ...
        val charges = EngineRuntime.chargeOptions(context, id).get(120, TimeUnit.SECONDS)
        return obj("options" to StateTestJson.options(value), "stats" to ModuleTestJson.stats(fit(id)),
            "modules" to array(details.modules.map { row -> obj("index" to row.index, "id" to row.id,
                "slot" to row.slot.name, "state" to if (row.id == null) null else row.state.name,
                "charge_id" to charges.modules[row.index].chargeId) }),
    ...
    private fun screenshot(name: String) {
        ...
        ParcelFileDescriptor.AutoCloseInputStream(automation.executeShellCommand(
            "screencap -p /sdcard/Download/pyfa-b052-$name.png")).use { it.readBytes() }
    }
    private fun retain(value: JSONObject, phase: String) {
        val descriptors = InstrumentationRegistry.getInstrumentation().uiAutomation.executeShellCommandRw(
            "dd of=/sdcard/Download/pyfa-b052-$phase.json")
    ...
    private fun open(id: String) {
        ...
        sync(); click("modules-open"); click("modules-bulk-states")
        waitFor { !picker.loading && picker.options?.fitId == id }
    }
    private fun apply() {
        click("bulk-states-apply")
    ...
    private fun close() { click("bulk-states-back"); click("modules-back") }
    private fun operation(id: String, input: JSONObject) = BridgeOperation.SetBulkStates(id, input.getInt("main_position"),
        input.getJSONArray("module_indices").let { values -> (0 until values.length()).map(values::getInt) },
        BulkScope.valueOf(input.getString("scope")), StateClick.entries.single { it.wire == input.getString("click") })
    private fun guards(id: String): JSONArray {
        val results = JSONArray()
        val raw = EngineRuntime.bulkStateOptionsDiagnostics(context, id).get(120, TimeUnit.SECONDS)
        BridgeCodec.decodeBulkStateOptions(raw)
        fun reject(name: String, pattern: String, replacement: String) {
            ...
            try { BridgeCodec.decodeBulkStateOptions(changed); fail("Accepted $name") }
        ...
        reject("invalid_state", "\"state\":\\s*\"[A-Z]+\"", "\"state\":\"SPINNING\"")
        reject("duplicate_candidate", "\"similar_candidates\":\\s*\\[[^]]*]", "\"similar_candidates\":[0,0]")
        ...
        reject("missing_click", "\"click_states\":\\s*\\{", "\"click_states\":{\"left_missing\":\"ONLINE\",")
        reject("extra_field", "\\{", "{\"extra\":1,")
        return results
    }

    @Test fun bulkStateTransitionsSurviveRestartOffline() {
        val phase = InstrumentationRegistry.getArguments().getString("b052_phase") ?: error("Missing phase")
        ...
        val fixture = JSONObject(InstrumentationRegistry.getInstrumentation().context.assets.open("bulk-states-expected.json")
        ...
        val saved = File(context.noBackupFilesDir, "b052-test-expected.json")
        try {
            when (phase) {
                "prepare" -> {
                    ...
                    for (c in 0 until cases.length()) {
                        ...
                        if (c == 5) { open(source); compose.onNodeWithTag("bulk-states-empty").assertExists(); screenshot("empty"); close() }
                    ...
                    val ship = create("Rifter", "B05 bulk states – 探索", listOf(
                        ModuleSpec("150mm Light AutoCannon II", ModuleState.ACTIVE, "EMP S"),
                        ModuleSpec("150mm Light AutoCannon II", ModuleState.OVERHEATED, "EMP S"),
                        ModuleSpec("200mm AutoCannon II", ModuleState.ACTIVE, "EMP S")))
                    send(BridgeOperation.SetFitRestrictions(ship, false), listOf(ship))
                    open(ship); click("bulk-states-reference-0")
                    assertEquals(setOf(0), picker.selected)
                    screenshot("selection")
                    compose.activityRule.scenario.recreate(); sync()
                    assertEquals(setOf(0), picker.selected); assertEquals(0, picker.main)
                    click("bulk-states-scope-SIMILAR"); click("bulk-states-click-OVERHEAT")
                    compose.onNodeWithTag("bulk-states-preview-2").performScrollTo().assertTextContains("→", substring = true)
                    compose.onNodeWithTag("bulk-states-preview").performScrollTo(); screenshot("similar")
                    apply()
                    assertEquals(listOf(ModuleState.OVERHEATED, ModuleState.OVERHEATED, ModuleState.OVERHEATED),
                        fit(ship).modules.take(3).map { it.state })
                    waitFor { picker.actualChanges.isNotEmpty() }
                    compose.onNodeWithTag("bulk-states-actual").performScrollTo().assertIsDisplayed()
                    screenshot("saved")
                    click("bulk-states-scope-SELECTED"); click("bulk-states-click-OFFLINE")
                    compose.onNodeWithTag("bulk-states-preview-1").performScrollTo().assertTextContains("Skipped", substring = true)
                    apply(); assertEquals(ModuleState.OFFLINE, fit(ship).modules[0].state)
                    assertEquals(ModuleState.OVERHEATED, fit(ship).modules[1].state)
                    codec = guards(ship)
                    val committed = library(); val priorOptions = allOptions()
                    for ((label, main, selected) in listOf(Triple("vacancy", 3, listOf(3)), Triple("missing_reference", 0, listOf(1)),
                            Triple("duplicate", 0, listOf(0, 0)), Triple("negative", -1, listOf(0)), Triple("out_of_range", 999, listOf(0)))) {
                        val result = send(BridgeOperation.SetBulkStates(ship, main, selected, BulkScope.SELECTED, StateClick.CYCLE), listOf(ship), false)
                        rejections.put(obj("label" to label, "code" to result.error?.code?.name))
                    }
                    val stale = EngineRuntime.request(context, BridgeOperation.SetBulkStates(ship, 0, listOf(0), BulkScope.SELECTED, StateClick.CYCLE),
                        mapOf(ship to 0L)).get(120, TimeUnit.SECONDS)
                    ...
                    val original = typed(report(ship))
                    val copy = send(BridgeOperation.DuplicateFit(ship, "B05 independent state copy"), listOf(ship)).fits.single {
                        it.name == "B05 independent state copy" }.id
                    send(BridgeOperation.SetBulkStates(copy, 0, listOf(0), BulkScope.SELECTED, StateClick.OVERHEAT), listOf(copy))
                    compare(original, typed(report(ship)))
                    send(BridgeOperation.SetModuleStates(ship, listOf(0), ModuleState.ONLINE), listOf(ship))
                    waitFor { !picker.loading && picker.options?.revision == fit(ship).revision && picker.selected.isEmpty() }
                    assertNull(picker.main)
                ...
                "restored" -> {
                    ...
                    compare(expected.getJSONObject("options"), allOptions(), "restored_state_types", strict = false)
                    ...
                    for (i in 0 until ids.length()) {
                        open(ids.getString(i)); click("bulk-states-select-all")
                        compose.onNodeWithTag("bulk-states-reference-0").performScrollTo().assertIsDisplayed()
            ...
            retain(obj("task" to "B05.2", "phase" to phase, "pid" to Process.myPid(), "runtime_start" to runtimeStart,
                ...
                "checks" to array(if (phase == "prepare") listOf("complete_matrix", "all_four_recipients", "prior_fits_unchanged",
                    ...
                    "empty_fit", "stale_selection_discarded", "fit_switch", "independent_copy", "atomic_rejections",
                    "recent_unchanged", "typed_protocol_guards") else listOf(
                    "fresh_process_restore", "identities_inputs_values_retained", "state_types_and_recent_retained", "reopen_each_fit"))), phase)
