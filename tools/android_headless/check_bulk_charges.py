"""Guarded original bulk charge selection, atomic edits, copies and real restart."""
import argparse
import ast
...
def main():
    ...
    fixture = ROOT / 'tools/android_reference/fixtures/bulk-charges.json'
    ...
    if not args.worker and not args.restore:
        ...
        evidence.update({'task': 'B05.1', 'host_only': True, 'database_sha256': before,
    ...
    def saved(bridge):
        return {'fits': json.loads(bridge.bootstrap())['fits'], 'recent': bridge.recent_items(),
                'options': [bridge.bulk_charge_options(key) for key in bridge._fits],
    ...
    class BulkChargeTests(unittest.TestCase):
        ...
        def dispatch(self, operation, arguments, ok=True, revision=None):
            ...
            response = json.loads(self.bridge.dispatch(json.dumps({'version': 1, 'session_id': self.bridge.session_id,
                'request_id': 'bulk-charges', 'operation': operation, 'arguments': arguments,
        ...
        def create(self, case=None):
            case = case or {'name': 'Empty bulk probe', 'ship': 'Rifter'}
        ...
        def change(self, key, step=1, ok=True):
            return self.dispatch('set_bulk_charges', {'fit_id': key, **expected['cases'][0]['steps'][step]['input']}, ok)
        def test_original_selection_commands_and_every_recipient(self):
                ...
                with self.subTest(case=case['name']):
                    ...
                    for step in case['steps']:
                        ...
                        if operation:
                            ...
                            response = self.dispatch('set_bulk_charges', {'fit_id': key, **operation})
                        ...
                        options = self.bridge.bulk_charge_options(key)
        ...
        def test_atomic_malformed_selection_scope_charge_and_stale_rejections(self):
            ...
            for field, values in {
                    ...
                    'scope': ['ALL', '', True, None],
                    'charge_id': [True, 1.0, '1', 0, -1, 99999999, engine._item('EMP S').ID]}.items():
                for value in values:
                    self.dispatch('set_bulk_charges', {**args, field:value}, False)
                    self.assertEqual(before, saved(self.bridge))
            self.dispatch('set_bulk_charges',args,False,revision=0)
            self.assertEqual(before,saved(self.bridge))
        def test_asymmetric_filter_similar_variants_and_no_unintended_edits(self):
            key = self.create(expected['cases'][0])
            self.change(key)
            rows = self.bridge._snapshots[key]['modules']
            self.assertEqual(['Spike M','Iron Charge M','Spike M','Scan Resolution Script'],[r['charge'] for r in rows[:4]])
            # Reference is Tech I: the desktop excludes the Tech II guns even
            # though every gun accepts Iron Charge M.
            self.change(key,2)
            self.assertEqual(['Spike M','Iron Charge M','Spike M','Scan Resolution Script'],
                             [r['charge'] for r in self.bridge._snapshots[key]['modules'][:4]])
            self.change(key,3)
            self.assertEqual(['Antimatter Charge M']*3+['Scan Resolution Script'],
                             [r['charge'] for r in self.bridge._snapshots[key]['modules'][:4]])
        def test_no_op_acknowledgment_retains_inputs_history_modified_order(self):
            key = self.create(expected['cases'][0]); self.change(key,3)
            before = saved(self.bridge); self.change(key,3)
            for row in before['fits']: row['revision'] += 1
            for row in before['options']: row['revision'] += 1
            self.assertEqual(before,saved(self.bridge))
        def test_read_only_empty_view_and_worker_ownership(self):
            key = self.create(); before = saved(self.bridge)
            self.assertEqual([],self.bridge.bulk_charge_options(key)['items'])
            self.assertTrue(all(not row['selection_candidates'] and not row['similar_candidates']
                                for row in self.bridge.bulk_charge_options(key)['modules']))
            self.assertEqual(before,saved(self.bridge))
            failures = []
            def foreign():
                try: self.bridge.bulk_charge_options(key)
        ...
        def test_failed_writes_independent_copies_and_real_process_restore(self):
            ...
            copy = next(row['id'] for row in self.dispatch('duplicate_fit',{'fit_id':key,'name':'Independent bulk copy'})['fits'] if row['id'] != key)
            self.change(copy,3)
            self.assertEqual(original,self.bridge._records[key])
            before = saved(self.bridge),(args.output/'fits.db').read_bytes()
            with patch.object(self.bridge._store,'write',side_effect=OSError('synthetic bulk save failure')):
    ...
    result = unittest.TextTestRunner(verbosity=2).run(unittest.defaultTestLoader.loadTestsFromTestCase(BulkChargeTests))
    if result.testsRun != 6 or not result.wasSuccessful() or result.skipped or result.expectedFailures or guard.attempts or network:
        raise RuntimeError('Bulk charge regressions failed or attempted forbidden access')
