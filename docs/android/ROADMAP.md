...
| B05.1 | active | B04 | Bulk ammunition selection | Selection-only and all-similar touch scopes expose exact targets/skips; original mixed-compatibility rules, atomic save/recovery and native restart pass. |
| B05.2 | queued | B05.1 | Bulk module states | Native multi-selection exposes original cycle/offline/overheat behavior and supported state fallbacks; recipients, restrictions and restart match desktop. |
